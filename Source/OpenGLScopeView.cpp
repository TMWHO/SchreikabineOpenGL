...
const char* const OpenGLScopeView::vertexShader = R"glsl(
    #ifdef GL_ES
...
const char* const OpenGLScopeView::fragmentShader = R"glsl(
    #ifdef GL_ES
    precision mediump float;
    #endif
    uniform vec4 colour;
...
OpenGLScopeView::OpenGLScopeView(AudioState& state) : audioState(state)
{
    setOpaque(true);
...
void OpenGLScopeView::setRenderData(const std::vector<float>& values)
{
...
void OpenGLScopeView::paint(juce::Graphics& g)
{
...
void OpenGLScopeView::newOpenGLContextCreated()
{
...
void OpenGLScopeView::openGLContextClosing()
{
    if (lineBuffer != 0)
...
void OpenGLScopeView::renderOpenGL()
{
    copyPendingData();
...
void OpenGLScopeView::copyPendingData()
{
    const juce::ScopedLock lock(dataLock);
...
void OpenGLScopeView::syncVisualParams()
{
...
float OpenGLScopeView::getCurrentSpectrumPeak() const
{
    float peak = 0.0f;
...
void OpenGLScopeView::updateSpectrumVerticesAndSpawn(int width, int height, float scale)
{
...
void OpenGLScopeView::spawnParticlesFromSpectrum(int width, int height)
{
...
void OpenGLScopeView::updateParticles(int height, float scale)
{
    if (audioState.particleRenderMode.load() == 0)
    {
        particles.clear();
...
void OpenGLScopeView::buildParticleVertices()
{
    particleVertices.clear();
...
void OpenGLScopeView::buildMeshVertices()
{
    meshVertices.clear();
...
float OpenGLScopeView::frequencyToX(float freq, float width)
{
...
void OpenGLScopeView::uploadBuffer(GLuint buffer, const std::vector<LineVertex>& vertices)
{
...
void OpenGLScopeView::uploadBuffer(GLuint buffer, const std::vector<ParticleVertex>& vertices)
{
    juce::gl::glBindBuffer(juce::gl::GL_ARRAY_BUFFER, buffer);
    juce::gl::glBufferData(juce::gl::GL_ARRAY_BUFFER,
...
void OpenGLScopeView::drawLine(float red, float green, float blue, float alpha, float lineWidth)
{
    shader->setUniform("pointMode", 0.0f);
    shader->setUniform("particleHardMode", 0.0f);
    shader->setUniform("pointSize", 1.0f);
...
void OpenGLScopeView::drawGlowPoints(float scale)
{
...
void OpenGLScopeView::drawLiveSpectrumPoints(float scale)
{
    shader->setUniform("pointMode", 0.0f);
...
void OpenGLScopeView::drawParticleDebugAnchors(float scale)
{
    shader->setUniform("pointMode", 1.0f);
    shader->setUniform("particleHardMode", 1.0f);
...
void OpenGLScopeView::drawLineBuffer(GLuint buffer, GLenum primitive, int count)
{
    juce::gl::glBindBuffer(juce::gl::GL_ARRAY_BUFFER, buffer);
...
void OpenGLScopeView::drawBuffer(GLuint buffer, GLenum primitive, int count)
{
    juce::gl::glBindBuffer(juce::gl::GL_ARRAY_BUFFER, buffer);
    juce::gl::glEnableVertexAttribArray(positionAttribute->attributeID);
    juce::gl::glVertexAttribPointer(positionAttribute->attributeID, 3, juce::gl::GL_FLOAT,
